...
# inum is created in the csv file before importing
...
# write_csv(item.tbl1, "output/item.tbl.whole.sample.csv")
...
A histogram and a boxplot below show the distribution of the student ability estimates. As can be seen, this distribution is much more streched across the logit scale from -3.5 to almost 4.0 (compared to the item difficulty range: -1.57 to 1.54). The red line in the boxplot shows the mean person ability, which is constraint to 0 for the model identification purpose. 
...
inum <- item_df %>% 
  select(formAndId, inum) 

rdf<- read_csv("data/2024-06-13/cva-trials-filtered-2024-06-13-deidentified.csv") %>%
  mutate(Form = ifelse(is.na(Form), "b-5-6", Form),
         itemId = ifelse(is.na(itemId), "19m", itemId)) %>% 
  mutate(formAndId = paste(Form, itemId, sep = "-")) %>% 
  select(user.assessmentUid, formAndId, item, response, target) 

rdf <- left_join(rdf, inum)

# key needs to be fromatted as matrix
cva.key <- distinct(rdf, inum, target, .keep_all = TRUE) %>% 
  arrange(inum) %>% select(target) %>% as.matrix()

rdf <- rdf %>% 
  select(sid = user.assessmentUid, inum, response) %>% 
  pivot_wider(names_from = inum, values_from = response)
...
library(purrr)
ws.DA.df <- map_df(ws.DA, ~as.data.frame(.x), .id="inum") %>% 
  group_by(inum) %>%
  filter(n != 0)

# get decorate column
decorated <- fm.id %>% 
  select(inum, decorated)

# merge decorate column, clean up the table
ws.DA.df <- 
  left_join(ws.DA.df, decorated) %>% 
  select(inum, decorated, correct,
...
ws.DA.df <- ws.DA.df %>% 
  arrange(inum, desc(correct), options)
...
# write_csv(ws.DA.df, "cva.distractor.whole.sample.csv")
